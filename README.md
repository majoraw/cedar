...
This research aims to develop an alternative approach based on blockchain and distributed technologies to addressing security challenges in microservices applications. The output of the research is an easy to use but comprehensive framework that improves the security resilience of microservices applications. Following a detailed literature review, the research has established that:
• observation: recent advances in the scalability, reliability and security
of blockchain offers great potential for cyber security use cases that make
it easier for engineers to build microservices applications that are more
resilient to cyber attacks.
• systematic literature review: the distributed nature of microservices
architecture poses significant security challenges that are difficult to com-
pletely solve with current approaches hence the need to look at alternatives
that are based on blockchain and distributed technologies.
• hypothesis: will prove that microservice applications that are developed
using blockchain based approaches are more resilient to cyber attacks than
those developed with current approaches.
• control application: create a simple microservices application using cur-
rent protocols and run benchmark tests
...
Terminology
Microservices - Generally, we use this term to refer to an architectural pattern where business logic is encapsulated in a bounded context, the unit of service is minimalist and services are consumed in an event driven manner.
Microservices function or simply function - This refers to the smallest unit that encapsulates business logic or 3rd party library. As a best practice, we advocate clear separation of business logic from proprietary libraries. By wrapping 3rd party libaries as functions, we can keep the event API intact when switching to an alternative library.
Microservices module or microservice - This refers to a single unit of deployment that contains at least one public function and optionally some private functions. All functions communicate with each others through a memory based event stream system within the module. Each microservices module can be independently deployed and scaled.
Microservices application - It is a collection of microservices modules running together as a single application.
User facing endpoints - This refers to public API for REST, websocket or other communication protocols.
Event API - Microservices functions expose event APIs internally for inter-service communications. For example, a user facing endpoint may use REST protocol. The HTTP request is then converted to an event to a microservice.
Real-time events - Messages that are time sensitive. e.g. RPC requests and responses.
Store-n-forward events - Events that are not time sensitive. First, a calling function can send an event without waiting for a response. Second, the called function may not even available when the event is delivered. e.g. data pipeline applications.
Streaming events - This refers to continuous flow of events from one function to another. Note that streaming can be either real-time or store-n-forward.
public function - This means the function is visible to all application containers and instances in the system.
private function - This means the function is visible only to functions in the same memory space inside an application instance.
route name - Each service can be uniquely identified with a name.
closed user groups - Conceptually, we treat the underlying event stream system as a conduit where one group of application instances can be logically separated from another group.
Post Office
...
system platform-core and rest-spring
connectors kafka, hazelcast, ...
language packs python, node.js...
extensions rest-automation, api-playground, ...
documentation architecture and user guides
examples code samples / templates for creating microservices with or without REST endpoints
...
High level of decoupling - You may write business logic as anonymous functions that run concurrently and independently. In addition to business logic, you may also encapsulate platform components and databases as anonymous functions.
Event driven programming - Each function is addressable with a unique "route name" and they communicate by sending events. You make request from one function to another by making a service call through some simple Mercury Post Office API.
One or more functions can be packaged together as a microservice executable, usually deployed as a Docker image or similar container technology.
rest-spring
...
cd rest-example
mvn clean package
java -jar target/rest-example-2.5.0.jar
...
mvn clean package
java -jar target/kafka-standalone-2.5.0.jar
...
mvn clean package
java -jar target/kafka-presence-2.5.0.jar
...
cd mercury/connectors/kafka/kafka-presence/
mvn clean package
java -Dserver.port=8081 -jar target/kafka-presence-2.5.0.jar
...
java -Dcloud.connector=kafka -jar target/lambda-example-2.5.0.jar
...
java -Dcloud.connector=kafka -jar target/rest-example-2.5.0.jar
